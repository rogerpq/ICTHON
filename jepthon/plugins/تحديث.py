...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**✫︙قام مطورين السورس بتحديث ريبثون**\n✫︙**التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ✫︙تم تحديث سورس ريبثون بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**✫︙يـتـم البـحـث عـن تـحديثـات سـورس ريبثون انـتـظـر**")
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "**✫︙سورس ريبثون محدث الى اخر اصدار **\n"
            f"**قـنـاة سـورس ريبثون** : @Repthon"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس ريبثون ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ✫︙جار تحـديـث سـورس ريبثون انـتـظـر قـليـلا 🔨**")
