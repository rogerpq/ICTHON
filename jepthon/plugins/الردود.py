...
async def filter_incoming_handler(event):
    ...
    rozrtba = (
        ".「  مطـور السورس  」."
        if userid == 5256894970
        else (".「  العضـو  」.")
    )
    rozrtba = (
        ".「 مـالك الحساب  」."
        if userid == (await event.client.get_me()).id
        and userid != 5256894970
        else rozrtba
    )
...
@jepiq.ar_cmd(pattern="ايقاف ([\s\S]*)")
...
@jepiq.ar_cmd(pattern="حذف الردود$")
